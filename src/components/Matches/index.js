...
class Matches extends Component {
  ...
  finishGame = score => {
    ...
    this.setState({isGameInProgress: false, score: newScore})
  }
  clickImage = id => {
    const {score, randomList} = this.state
    if (id === randomList.id) {
      this.setState(prevState => ({
  ...
  render() {
    const {isGameInProgress, timeInSeconds, score} = this.state
    return (
      <div className="app-container">
        <ul className="navbar-container">
          <Navbar score={score} timeInSeconds={timeInSeconds} />
        </ul>
        {isGameInProgress ? this.renderMatchItem() : this.renderMatchCard()}
      </div>
    )
  }
}
